...
# Simulation 1: BART, BAT, F1, step-wise frequency

The first simulation models the effects of an enrouching category affected by a bias on the category towards which the former is moving. In our case, the encrouching category is the the BART vowel, while the other category is the BAT vowel.

## Functions

The following chunks define some functions that will be used to initialise a lexicon of words containing either a BART or a BAT vowel.
...
The function `populateLexicon()` adds formant values to the lexical items in the lexicon, following a normal distribution. It follows the principles outlined in @pierrehumbert2001 and @wedel2007. The chosen standard values for F1 are 6.5 Bark for BART and 5 for BAT.

```{r populateLexicon}
populateLexicon <- function(lexicon, iterations, probs, bart.f1, bat.f1) {
    for (i in 1:iterations) {
        item <- sample(seq(1, length(lexicon)), 1, prob = probs)
        cloud.size <- length(lexicon[[item]][[4]])
        if (item %% 2 != 0) {
            if (cloud.size == 0) {
                lexicon[[item]][[4]][[1]] <- produceFormants(bart.f1)
...
We can now populate a lexicon with exemplars for BART and BAT vowels.

```{r lexicon}
set.seed(1234)

lexicon.size <- 100
frequency <- rep(1:lexicon.size, each = 2, len = lexicon.size)
group.size <- 5
...
bart.f1 <- 6.5
bat.f1 <- 5.5
lexicon <- populateLexicon(lexicon, iterations, frequency, bart.f1, bat.f1)
...
for (iteration in 1:iterations) {
    ...
    if (word %% 2 != 0) { # odd words have "BART"

        #### The chosen word is a BART word ####

        if (length(lexicon[[word]][[4]]) == 0) {
            lexicon[[word]][[4]] <- list(produceFormants(bart.f1, bias))
        } else {
        ...
        if (length(lexicon[[word]][[4]]) == 0) {
            lexicon[[word]][[4]] <- list(produceFormants(bat.f1))
        } else {
            target <- sample(1:length(lexicon[[word]][[4]]), 1)
    ...
    if (iteration %% 1000 == 0) {
        ...
        for (word in 1:lexicon.size) {
            ...
            if (is.null(unlist(lexicon[[word]][[4]])) == FALSE) {
        ...
        vowelss <- rep(c("BART", "BAT"), len = lexicon.size)
...
for (i in seq(1, 100, by = 2)) {
    bart.source <- c(bart.source, unlist(source.lexicon[[i]][[4]]))
}
...
vowels.source <- data.frame(
    f1 = c(bart.source, bat.source),
    vowel = c(rep("BART", times = length(bart.source)), rep("BAT", times = length(bat.source)))
)
...
bart <- NULL
for (i in seq(1, 100, by = 2)) {
    bart <- c(bart, unlist(lexicon[[i]][[4]]))
}
...
vowels <- data.frame(
    f1 = c(bart, bat),
    vowel = c(rep("BART", times = length(bart)), rep("BAT", times = length(bat)))
)
...
BART and BAT words, F1 only.

```{r s1-lmer}
change.scaled <- mutate(change, frequency = scale(frequency), time = scale(time))
...
simulation1.gam <- bam(
    f1 ~
        ...
        s(time, bs = "cr", by = vowel.ord) +
        s(frequency, bs = "cr") +
        ti(time, frequency) +
        ti(time, frequency, by = vowel.ord) +
        s(time, word.ord, bs = "fs"),
    data = change,
    method = "ML"
)

rho <- start_value_rho(simulation1.gam)
...
```{r simulation1-gam-plot-bart}
fvisgam(simulation1.gam.ar, view = c("time","frequency"),
        cond = list(vowel.ord = "BART"),
        rm.ranef = TRUE
        )
```

```{r simulation1-gam-plot-bat}
fvisgam(simulation1.gam.ar, view = c("time","frequency"),
        cond = list(vowel.ord = "BAT"),
        rm.ranef = TRUE
        )
```

```{r simulation1-gam-acr}
acf_plot(resid(simulation1.gam.ar), split_by=list(change$word.ord))
acf_resid(simulation1.gam.ar, split_pred="AR.start")
```

```{r change-smooths-BAT}
plot_smooth(simulation1.gam.ar, view="time", cond=list(frequency=5, vowel.ord = "BAT"),
            rug=F, col="red")
plot_smooth(simulation1.gam.ar, view="time", cond=list(frequency=25, vowel.ord = "BAT"),
            rug=F, col="green", add=T)
plot_smooth(simulation1.gam.ar, view="time", cond=list(frequency=45, vowel.ord = "BAT"),
            rug=F, col="blue", add=T)
```



# Simulation 2: BART, BAT, BET, F1, F2

```{r createLexicon2}
createLexicon2 <- function(size, frequency, group.size) {

    word <- seq(1, size)
    vowel <- rep(c("BART", "BAT", "BET"), size)
    class <- sample(rep(1:(size/group.size), each = group.size))
#    class <- c(rbind(sample(1:(size/group.size)), sample(1:(size/group.size))))
    cloud.f1 <- list()
    cloud.f2 <- list()
...
```{r populateLexicon2}
populateLexicon2 <- function(lexicon, iterations, probs, bart.f1, bart.f2, bat.f1, bat.f2, bet.f1, bet.f2) {
    for (i in 1:iterations) {
        item <- sample(seq(1, length(lexicon)), 1, prob = probs)
        cloud.size <- length(lexicon[[item]][[4]])
        if (cloud.size == 0) {
            if (lexicon[[item]][[3]] == "BART") {
                f1 <- bart.f1
                f2 <- bart.f2
            } else if (lexicon[[item]][[3]] == "BAT") {
                f1 <- bat.f1
                f2 <- bat.f2
            } else {
                f1 <- bet.f1
...
bet.f2 <- 13.3
system.time(
lexicon2 <- populateLexicon2(lexicon2, iterations, frequency, bart.f1, bart.f2, bat.f1, bat.f2, bet.f1, bet.f2)
)
...
for (iteration in 1:iterations) {
    ...
    if (lexicon2[[word]][[3]] == "BART") {

        #### The chosen word is a BART word ####

        if (length(lexicon2[[word]][[4]]) == 0) {
            lexicon2[[word]][[4]] <- list(produceFormants(bart.f1, f1.bias))
            lexicon2[[word]][[5]] <- list(produceFormants(bart.f2, f2.bias))
        } else {
    ...
    } else if (lexicon2[[word]][[3]] == "BAT") {

        #### The chosen word is a BAT word ####

        if (length(lexicon2[[word]][[4]]) == 0) {
            lexicon2[[word]][[4]] <- list(produceFormants(bat.f1))
            lexicon2[[word]][[5]] <- list(produceFormants(bat.f2))
        } else {
            target <- sample(1:length(lexicon2[[word]][[4]]), 1)
    ...
    if (iteration %% 1000 == 0) {
        ...
        for (word in 1:lexicon.size) {
            ...
            now.frequency <- c(now.frequency, word.frequency)
            if (is.null(unlist(lexicon2[[word]][[4]])) == FALSE) {
                word.f1.mean <- mean(unlist(lexicon2[[word]][[4]]))
            } else {
                word.f1.mean <- NA
            }
            f1.mean <- c(f1.mean, word.f1.mean)
            if (is.null(unlist(lexicon2[[word]][[5]])) == FALSE) {
        ...
        vowelss <- rep_len(c("BART", "BAT", "BET"), lexicon.size)
        words <- seq(1, lexicon.size)
        time <- rep(iteration, lexicon.size)
...
for (i in seq(1, 1000, by = 3)) {
    bart.f1.source <- c(bart.f1.source, unlist(source.lexicon2[[i]][[4]]))
}
bart.f2.source <- NULL
for (i in seq(1, 1000, by = 3)) {
    bart.f2.source <- c(bart.f2.source, unlist(source.lexicon2[[i]][[5]]))
}
...
vowels.source2 <- data.frame(
    f1 = c(bart.f1.source, bat.f1.source, bet.f1.source),
    f2 = c(bart.f2.source, bat.f2.source, bet.f2.source),
    vowel = c(rep("BART", times = length(bart.f1.source)), rep("BAT", times = length(bat.f1.source)), rep("BET", times = length(bet.f1.source)))
)
...
bart.f1.final <- NULL
for (i in seq(1, 1000, by = 3)) {
    bart.f1.final <- c(bart.f1.final, unlist(lexicon2[[i]][[4]]))
}
bart.f2.final <- NULL
for (i in seq(1, 1000, by = 3)) {
    bart.f2.final <- c(bart.f2.final, unlist(lexicon2[[i]][[5]]))
}
...
vowels.final2 <- data.frame(
    f1 = c(bart.f1.final, bat.f1.final, bet.f1.final),
    f2 = c(bart.f2.final, bat.f2.final, bet.f2.final),
    vowel = c(rep("BART", times = length(bart.f1.final)), rep("BAT", times = length(bat.f1.final)), rep("BET", times = length(bet.f1.final)))
)
...
BART, BAT and BET words, F1 and F2
...
This Simulation shows that high frequency words lead the change in the encroaching category (BART), while low frequency words do in the encroached on categories (BAT, BET).
...
```{r populateLexicon3}
populateLexicon3 <- function(lexicon, iterations, probs, bart.f1, bat.f1, bet.f1) {
    for (i in 1:iterations) {
        item <- sample(seq(1, length(lexicon)), 1, prob = probs)
        cloud.size <- length(lexicon[[item]][[4]])
        if (cloud.size == 0) {
...
lexicon.size <- 100
frequency <- rep(1:lexicon.size, each = 3, len = lexicon.size)
group.size <- 10
...
bat.f1 <- 5.5
bet.f1 <- 4.5
lexicon3 <- populateLexicon3(lexicon3, iterations, frequency, bart.f1, bat.f1, bet.f1)
...
for (iteration in 1:iterations) {
    ...
    if (lexicon3[[word]][[3]] == "BART") {

        #### The chosen word is a BART word ####

        if (length(lexicon3[[word]][[4]]) == 0) {
            lexicon3[[word]][[4]] <- list(produceFormants(bart.f1, bias))
        } else {
    ...
    } else if (lexicon3[[word]][[3]] == "BAT") {

        #### The chosen word is a BAT word ####

        if (length(lexicon3[[word]][[4]]) == 0) {
            lexicon3[[word]][[4]] <- list(produceFormants(bat.f1))
        } else {
            target <- sample(1:length(lexicon3[[word]][[4]]), 1)
        ...
        for (i in 1:group.size){
            other.word <- groups3[[class]][[2]][[i]]
            if (lexicon3[[other.word]][[3]] == "BART") {
                pool.a <- c(pool.a, unlist(lexicon3[[other.word]][[4]]))
            } else if (lexicon3[[other.word]][[3]] == "BET") {
    ...
    if (iteration %% 1000 == 0) {
        ...
        for (word in 1:lexicon.size) {
            ...
            now.frequency <- c(now.frequency, word.frequency)
            if (is.null(unlist(lexicon3[[word]][[4]])) == FALSE) {
        ...
        vowelss <- rep(c("BART", "BAT", "BET"), len = lexicon.size)
        words <- seq(1, lexicon.size)
        time <- rep(iteration, lexicon.size)
        change.current <- data.frame("word" = words, time, "frequency" = now.frequency, "f1" = f1.mean, "vowel" = vowelss)
...
for (i in seq(1, 100, by = 3)) {
    bart.source <- c(bart.source, unlist(source.lexicon3[[i]][[4]]))
}
...
vowels.source <- data.frame(
    f1 = c(bart.source, bat.source, bet.source),
    vowel = c(rep("BART", times = length(bart.source)), rep("BAT", times = length(bat.source)), rep("BET", times = length(bet.source)))
)
...
bart <- NULL
for (i in seq(1, 100, by = 3)) {
    bart <- c(bart, unlist(lexicon3[[i]][[4]]))
}
...
vowels <- data.frame(
    f1 = c(bart, bat, bet),
    vowel = c(rep("BART", times = length(bart)), rep("BAT", times = length(bat)), rep("BET", times = length(bet)))
)
...
BART, BAT and BET words, F1 only.

```{r change5-scaled}
change3.scaled <- mutate(change3, frequency = scale(frequency), time = scale(time))
```
...
```{r s3-lmer-plot, eval=FALSE}
plot(allEffects(simulation3.lm))
```
...
compareML(simulation3.gam.ar, simulation3.gam.ar.null)
```
...
```{r simulation3-gam-smooth}
plot_smooth(simulation3.gam.ar, view="time", cond=list(frequency=5, vowel.ord = "BAT"),
            rug=F, col="red", rm.ranef = TRUE)
plot_smooth(simulation3.gam.ar, view="time", cond=list(frequency=20, vowel.ord = "BAT"),
            rug=F, col="green", add=T, rm.ranef = TRUE)
plot_smooth(simulation3.gam.ar, view="time", cond=list(frequency=30, vowel.ord = "BAT"),
            rug=F, col="blue", add=T, rm.ranef = TRUE)
```


# Simulation 4: BART, BAT, F1, F2
...
```{r populateLexicon4}
populateLexicon4 <- function(lexicon, iterations, probs, bart.f1, bart.f2, bat.f1, bat.f2) {
    for (i in 1:iterations) {
        item <- sample(seq(1, length(lexicon)), 1, prob = probs)
        cloud.size <- length(lexicon[[item]][[4]])
        if (cloud.size == 0) {
...
bat.f1 <- 5.5
bat.f2 <- 13
lexicon4 <- populateLexicon4(lexicon4, iterations, frequency, bart.f1, bart.f2, bat.f1, bat.f2)
...
for (iteration in 1:iterations) {
    ...
    if (lexicon4[[word]][[3]] == "BART") {

        #### The chosen word is a BART word ####

        if (length(lexicon4[[word]][[4]]) == 0) {
            lexicon4[[word]][[4]] <- list(produceFormants(bart.f1, f1.bias))
            lexicon4[[word]][[5]] <- list(produceFormants(bart.f2, f2.bias))
        } else {
        ...
        if (length(lexicon4[[word]][[4]]) == 0) {
            lexicon4[[word]][[4]] <- list(produceFormants(bat.f1))
            lexicon4[[word]][[5]] <- list(produceFormants(bat.f2))
        } else {
            target <- sample(1:length(lexicon4[[word]][[4]]), 1)
    ...
    if (iteration %% 1000 == 0) {
        ...
        for (word in 1:lexicon.size) {
            ...
            now.frequency <- c(now.frequency, word.frequency)
            if (is.null(unlist(lexicon4[[word]][[4]])) == FALSE) {
                word.f1.mean <- mean(unlist(lexicon4[[word]][[4]]))
            } else {
                word.f1.mean <- NA
            }
            if (is.null(unlist(lexicon4[[word]][[5]])) == FALSE) {
        ...
        vowelss <- rep_len(c("BART", "BAT"), lexicon.size)
        words <- seq(1, lexicon.size)
        time <- rep(iteration, lexicon.size)
        change.current <- data.frame("word" = words, time, "frequency" = now.frequency, "f1" = f1.mean, "f2" = f2.mean, "vowel" = vowelss)
...
for (i in seq(1, lexicon.size, by = 2)) {
    bart.f1.source <- c(bart.f1.source, unlist(source.lexicon4[[i]][[4]]))
}
bart.f2.source <- NULL
for (i in seq(1, lexicon.size, by = 2)) {
    bart.f2.source <- c(bart.f2.source, unlist(source.lexicon4[[i]][[5]]))
}
...
vowels.source4 <- data.frame(
    f1 = c(bart.f1.source, bat.f1.source),
    f2 = c(bart.f2.source, bat.f2.source),
    vowel = c(rep("BART", times = length(bart.f1.source)), rep("BAT", times = length(bat.f1.source)))
)
...
bart.f1 <- NULL
for (i in seq(1, lexicon.size, by = 2)) {
    bart.f1 <- c(bart.f1, unlist(lexicon4[[i]][[4]]))
}
bart.f2 <- NULL
for (i in seq(1, lexicon.size, by = 2)) {
    bart.f2 <- c(bart.f2, unlist(lexicon4[[i]][[5]]))
}
...
vowels4 <- data.frame(
    f1 = c(bart.f1, bat.f1),
    f2 = c(bart.f2, bat.f2),
    vowel = c(rep("BART", times = length(bart.f1)), rep("BAT", times = length(bat.f1)))
)
...
# Simulation 5: BART, BAT, BET, F1, step-wise frequency
...
```{r populateLexicon5}
populateLexicon5 <- function(lexicon, iterations, probs, bart.f1, bat.f1, bet.f1) {
    for (i in 1:iterations) {
        item <- sample(seq(1, length(lexicon)), 1, prob = probs)
        cloud.size <- length(lexicon[[item]][[4]])
        if (cloud.size == 0) {
...
bat.f1 <- 5.5
bet.f1 <- 4.5
lexicon5 <- populateLexicon5(lexicon5, iterations, frequency, bart.f1, bat.f1, bet.f1)
...
for (iteration in 1:iterations) {
    ...
    if (lexicon5[[word]][[3]] == "BART") {

        #### The chosen word is a BART word ####

        if (length(lexicon5[[word]][[4]]) == 0) {
            lexicon5[[word]][[4]] <- list(produceFormants(bart.f1, bias))
        } else {
    ...
    } else if (lexicon5[[word]][[3]] == "BAT") {

        #### The chosen word is a BAT word ####

        if (length(lexicon5[[word]][[4]]) == 0) {
            lexicon5[[word]][[4]] <- list(produceFormants(bat.f1))
        } else {
            target <- sample(1:length(lexicon5[[word]][[4]]), 1)
        ...
        for (i in 1:group.size){
            other.word <- groups5[[class]][[2]][[i]]
            if (lexicon5[[other.word]][[3]] == "BART") {
                pool.a <- c(pool.a, unlist(lexicon5[[other.word]][[4]]))
            } else if (lexicon5[[other.word]][[3]] == "BET") {
...
```{r change5-scaled}
change5.scaled <- mutate(change5, frequency = scale(frequency), time = scale(time))
```
...
compareML(simulation5.gam.ar.null, simulation5.gam.ar)
```
...
```{r change5-smooths-BAT}
plot_smooth(simulation5.gam.ar, view="time", cond=list(frequency=10, vowel.ord = "BAT"),
            rug=F, col="red")
plot_smooth(simulation5.gam.ar, view="time", cond=list(frequency=40, vowel.ord = "BAT"),
            rug=F, col="blue", add=T)
plot_smooth(simulation5.gam.ar, view="time", cond=list(frequency=80, vowel.ord = "BAT"),
            rug=F, col="green", add=T)
```

```{r change5-smooths-BET}
