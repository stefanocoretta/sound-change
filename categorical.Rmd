...
```{r}
lexicon.change %>%
    ggplot(aes(iteration, glot.prob, group = word)) +
    geom_line(stat = "smooth", method = "loess", alpha = 0.1, se = FALSE)
```
...
contrasts(lexicon.change$word.ord) <- "contr.treatment"
lexicon.change <- lexicon.change %>%
...
glot.gam <- bam(
    glot.prob ~
        frequency +
        s(iteration, bs = "cr") +
        s(iteration, by = frequency, bs = "cr") +
        s(word, bs = "fs"),
    data = lexicon.change,
    method = "fREML"
)
...
fvisgam(glot.gam.ar, view = c("iteration","frequency"),
        ylim = quantile(lexicon.change$frequency, c(0, 1))
        )
```
