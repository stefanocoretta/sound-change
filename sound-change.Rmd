...
```{r populateLexicon}
populateLexicon <- function(lexicon, iterations, probs, bat.f2, bet.f2) {
    for (i in 1:iterations) {
        item <- sample(seq(1, length(lexicon)), 1, prob = probs)
        cloud.size <- length(lexicon[[item]][[4]])
        if (item %% 2 != 0) {
            if (cloud.size == 0) {
                lexicon[[item]][[4]][[1]] <- produceFormants(bat.f2)
            ...
            if (cloud.size == 0) {
                lexicon[[item]][[4]][[1]] <- produceFormants(bet.f2)
            } else if (cloud.size == 1) {
                target <- lexicon[[item]][[4]][[1]]
                lexicon[[item]][[4]][[cloud.size + 1]] <- produceFormants(target)
            } else {
...
bat.f2 <- 6.5
bet.f2 <- 5.5
lexicon <- populateLexicon(lexicon, iterations, frequency, bat.f2, bet.f2)
